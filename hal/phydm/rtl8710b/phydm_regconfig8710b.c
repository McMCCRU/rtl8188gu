...
)
{
	...
	odm_config_rf_reg_8710b(p_dm, addr, data, RF_PATH_A, addr | maskfor_phy_set);

	PHYDM_DBG(p_dm, ODM_COMP_INIT, ("===> odm_config_rf_with_header_file: [RadioA] %08X %08X\n", addr, data));
}

void
odm_config_rf_radio_b_8710b(
	struct PHY_DM_STRUCT				*p_dm,
	u32					addr,
	u32					data
)
{
	...
	odm_config_rf_reg_8710b(p_dm, addr, data, RF_PATH_B, addr | maskfor_phy_set);
...
void
odm_config_bb_phy_reg_pg_8710b(
	struct PHY_DM_STRUCT	*p_dm,
	u32		band,
	u32		rf_path,
	u32		tx_num,
...
{
	/*dbg_print("odm_config_bb_phy_8710b(), addr = 0x%x, data = 0x%x\n", addr, data);*/
	if (addr == 0xfe)
#ifdef CONFIG_LONG_DELAY_ISSUE
		ODM_sleep_ms(50);
#else
