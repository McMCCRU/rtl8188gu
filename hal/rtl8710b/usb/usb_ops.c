...
void rtl8710bu_interrupt_handler(_adapter *padapter, u16 pkt_len, u8 *pbuf)
{
	...
	/*only for BCN_0*/
	if (pHalData->IntArray[0] & IMR_BCNDMAINT0_8710B)
#endif
	...
	if (pHalData->IntArray[1] & IMR_TXERR_8710B)
		RTW_INFO("===> %s Tx Error Flag Interrupt Status\n", __func__);
	if (pHalData->IntArray[1] & IMR_RXERR_8710B)
		RTW_INFO("===> %s Rx Error Flag INT Status\n", __func__);
	if (pHalData->IntArray[1] & IMR_TXFOVW_8710B)
		RTW_INFO("===> %s Transmit FIFO Overflow\n", __func__);
	if (pHalData->IntArray[1] & IMR_RXFOVW_8710B)
...
int recvbuf2recvframe(PADAPTER padapter, void *ptr)
{
	...
	pkt_cnt = GET_RX_STATUS_DESC_USB_AGG_PKTNUM_8710B(pbuf);
#endif
	...
	do {
		...
		rtl8710b_query_rx_desc_status(precvframe, pbuf);
...
void rtl8710bu_xmit_tasklet(void *priv)
{
	...
	while (1) {
		...
		ret = rtl8710bu_xmitframe_complete(padapter, pxmitpriv, NULL);
...
{
	pdvobj->HardwareType = HARDWARE_TYPE_RTL8710BU;
	RTW_INFO("CHIP TYPE: RTL8710BU\n");
}
