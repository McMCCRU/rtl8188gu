...
)
{
	...
	if (bCurrentIntMt != IntMtToSet) {
		...
		if (IntMtToSet) {
			...
			/* 2010.03.05. */
			/* */
			rtw_write32(Adapter, REG_INT_MIG_8710B, 0xff000fa0);/* 0x306:Rx, 0x307:Tx */
			pHalData->bInterruptMigration = IntMtToSet;
		} else {
			/* Reset all interrupt migration settings. */
			rtw_write32(Adapter, REG_INT_MIG_8710B, 0);
			pHalData->bInterruptMigration = IntMtToSet;
		}
	}
 ...
 * functions
 * ************************************************************ */
static void Init_ODM_ComInfo_8710b(PADAPTER	Adapter)
{
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(Adapter);
...
{
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(Adapter);
	struct PHY_DM_STRUCT *phydm = &(pHalData->odmpriv);
...
void rtl8710b_init_dm_priv(IN PADAPTER Adapter)
{
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(Adapter);
	struct PHY_DM_STRUCT		*podmpriv = &pHalData->odmpriv;

	Init_ODM_ComInfo_8710b(Adapter);
	odm_init_all_timers(podmpriv);

}

void rtl8710b_deinit_dm_priv(IN PADAPTER Adapter)
{
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(Adapter);
	struct PHY_DM_STRUCT		*podmpriv = &pHalData->odmpriv;
