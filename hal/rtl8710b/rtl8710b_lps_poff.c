...
*****************************************************************************/
static void rtl8710b_lps_poff_h2c_param(PADAPTER padapter, u8 tx_bndy, u16 len,
					bool isDynamic)
{
...
**************************************************************************/
static void rtl8710b_lps_poff_set_param(PADAPTER padapter)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	...
	rtl8710b_lps_poff_h2c_param(padapter, static_tx_bndy,
				    static_len, _FALSE);
	...
	if (dynamic_len != 0) {
		dynamic_len += 8;
		rtl8710b_lps_poff_h2c_param(padapter, dynamic_tx_bndy,
					    dynamic_len, _TRUE);
	}
...
*****************************************************************************/
static void rtl8710b_lps_poff_set_tx_bndy(PADAPTER padapter, u8 tx_bndy)
{
	u32	numHQ = 0x10;
	u32	numLQ = 0x10;
	u32	numPubQ = 0;
...
*****************************************************************************/
static bool rtl8710b_lps_poff_tx_bndy_flow(PADAPTER padapter, bool enable)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	...
	/* change tx boundary*/
	rtl8710b_lps_poff_set_tx_bndy(padapter, tx_bndy_new);
	...
	RTW_INFO("%s: free tail = %#x after\n", __func__,
		 rtw_read8(padapter, REG_FW_FREE_TAIL_8710B));
	...
	/* reinit LLT */
	rtl8710b_InitLLTTable(padapter);
...
*****************************************************************************/
static u8 rtl8710b_lps_poff_append_extra_info(PADAPTER padapter, u32 len)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	lps_poff_info_t *plps_poff_info = pwrpriv->plps_poff_info;
...
*****************************************************************************/
static void rtl8710b_lps_poff_send_config_frame(PADAPTER padapter,
		u8 *pFile, u32 len)
{
...
*****************************************************************************/
static void rtl8710b_lps_poff_send_config_file(PADAPTER padapter,
		u8 *pFile, u8 loc, u32 len)
{
	...
	rtw_write8(padapter, REG_TDECTRL_8710B + 1, loc);

	DLBcnCount = 0;
	poll = 0;

	do {
		rtl8710b_lps_poff_send_config_frame(padapter, pFile, len);
		DLBcnCount++;
		do {
			rtw_mdelay_os(10);
	...
	rtw_write8(padapter, REG_TDECTRL_8710B + 1, tx_bndy);
...
*****************************************************************************/
static void rtl8710b_lps_poff_dl_config_file(PADAPTER padapter)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	lps_poff_info_t *plps_poff_info = pwrpriv->plps_poff_info;
	...
	/*static file*/
	rtl8710b_lps_poff_send_config_file(padapter,
	...
	if (dynamic_file_len != 0) {
		...
		append_len =
			rtl8710b_lps_poff_append_extra_info(padapter,
					dynamic_file_len);
...
static u8 rtl8710b_lps_poff_set_dynamic_file(u8 *pFile, u32 type, u32 wdata)
{
...
static void rtl8710b_lps_poff_dynamic_file(PADAPTER padapter, u8 index, u8 isGK)
{
	...
	for (i = 0 ; i < CAM_CONTENT_COUNT; i++) {
		...
		ret = rtl8710b_lps_poff_set_dynamic_file(ptkfile,
				WCAMI, tgt_wdata);
...
static void rtl8710b_lps_poff_sec_cam_opt(PADAPTER padapter)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	lps_poff_info_t *plps_poff_info = pwrpriv->plps_poff_info;
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	...
	for (i = 0 ; i < cam_ctl->num ; i++) {
		...
		if (isValid && !isGK) {
			rtl8710b_lps_poff_dynamic_file(padapter, i, _FALSE);
	...
	rtl8710b_lps_poff_dynamic_file(padapter, gk_start, _TRUE);
...
*****************************************************************************/
static bool rtl8710b_prepare_for_enter_poff(PADAPTER padapter, bool bEnterLPS)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct cam_ctl_t *cam_ctl = &dvobj->cam_ctl;
	...
	ret = rtl8710b_lps_poff_tx_bndy_flow(padapter, bEnterLPS);

	if (ret == _TRUE) {
		if (cam_cache_num > 0) {
			rtl8710b_lps_poff_sec_cam_opt(padapter);
		} else {
			plps_poff_info->ConfLenForPTK = 0;
			plps_poff_info->ConfLenForGTK = 0;
		}
		rtl8710b_lps_poff_dl_config_file(padapter);
		rtl8710b_lps_poff_set_param(padapter);
	}
	return ret;
}
...
**************************************************************************/
void rtl8710b_lps_poff_h2c_ctrl(PADAPTER padapter, u8 en)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
...
void rtl8710b_lps_poff_set_ps_mode(PADAPTER padapter, bool bEnterLPS)
{
	...
	if (plps_poff_info->bEn) {
		...
		if (bEnterLPS) {
			res = rtl8710b_prepare_for_enter_poff(padapter,
							      bEnterLPS);
			ATOMIC_SET(&plps_poff_info->bEnterPOFF, res);
		} else
			rtl8710b_lps_poff_tx_bndy_flow(padapter, bEnterLPS);
	}
}
